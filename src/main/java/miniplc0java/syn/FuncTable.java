...
public class FuncTable {
    ...
    public void add(Function function) {
        for (int i = 1; i < functions.size(); i++) {
            if (functions.get(i).name.equals(function.name)) {
                throw new Error("function [" + function.name + "] redefined " + Prototype.getPos());
            }
        }
    ...
    public void addVariable(Variable variable) {
        if (variable.isGlobal) {
            ...
            throw new Error("cannot identify type of variable " + Prototype.getPos());
        }
    }
    ...
    public void addInstruction(Function function, String instructionString, byte... instruction) {
        ...
        for (byte b : instruction) {
            function.instructions.get(num).add(b);
        }
    ...
    public void replaceInstruction(int pos, String instructionString, byte... instruction) {
        ...
        for (byte b : instruction) {
            topFunction.instructions.get(pos).add(b);
        }
    }

    public int getInstructionNum() {
        return topFunction.instructionNum;
    }

    public Integer searchFunction(String name) {
        for (String s : Prototype.stdio) {
            if (s.equals(name)) {
                return null;
            }
        }
        ...
        throw new Error("cannot identify function [" + name + "] " + Prototype.getPos());
    }
}
